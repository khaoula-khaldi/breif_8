...
<body class="bg-purple-100 min-h-screen p-6">
    ...
    <h1 class="text-2xl font-bold text-gray-800 mb-6 text-center">
        Historique des transactions entre les cartes 
    </h1>
    ...
    <div class="overflow-x-auto bg-white rounded-xl shadow-lg">
        <table class="w-full border-collapse">
            ...
            <tbody>
                <?php
                    $stmt = $pdo->prepare("SELECT * FROM transactions WHERE id=? ORDER BY date_transaction DESC LIMIT 5");
                    ...
                    foreach ($transactions as $t) {
                        ...
                        <tr class='border-t hover:bg-gray-50'>
                            <td class='p-3'>{$t['date_transaction']}</td>
                            <td class='p-3'>Carte → Carte</td>
                            <td class='p-3'>{$t['carte_source_id']}</td>
                            <td class='p-3'>{$t['carte_dest_id']}</td>
